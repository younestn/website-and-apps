    ...
    return Consumer<ShopController>(
      builder: (context, seller, child) {
        final bool isVacationActive = ShopHelper.isVacationActive(
          context, startDate: seller.sellerInfoModel?.seller?.shop?.vacationStartDate,
          endDate: seller.sellerInfoModel?.seller?.shop?.vacationEndDate,
          vacationDurationType: seller.sellerInfoModel?.seller?.shop?.vacationDurationType,
          vacationStatus: seller.sellerInfoModel?.seller?.shop?.vacationStatus,
                ...
                Stack(children: [
                  Container(
                    ...
                    child: ClipRRect(
                      ...
                      child: CustomImageWidget(image: seller.sellerInfoModel?.seller != null
                          ? '${seller.sellerInfoModel?.seller?.shop?.imageFullUrl?.path}'
                          : "${Provider.of<SplashController>(context, listen: false).configModel?.inHouseShop?.imageFullUrl?.path}"),
                    ),
                  ),

                  if(isVacationActive || (seller.sellerInfoModel?.seller?.shop?.temporaryClose ?? false))
                      ...
                      Row(children: [
                        Expanded(child: InkWell(
                            ...
                            child: Column(mainAxisAlignment: MainAxisAlignment.start,
                              crossAxisAlignment: CrossAxisAlignment.start, children: [
                                Text(seller.sellerInfoModel != null ? seller.sellerInfoModel?.seller?.shop?.name ?? '${Provider.of<SplashController>(context, listen: false).configModel?.inHouseShop?.name}'  : '${Provider.of<SplashController>(context, listen: false).configModel?.inHouseShop?.name}',
                        ...
                        InkWell(
                          onTap: () {
                            if(!Provider.of<AuthController>(context, listen: false).isLoggedIn()) {
                            ...
                            }else if(seller.sellerInfoModel != null && ((seller.sellerInfoModel?.seller?.shop?.temporaryClose ?? false))){
                              ...
                              if(seller.sellerInfoModel?.seller != null){
                                Navigator.push(context, MaterialPageRoute(builder: (_) => ChatScreen(
                                  ...
                                  isShopTemporaryClosed: seller.sellerInfoModel?.seller?.shop?.temporaryClose ?? false,
                                )));
                              }else{
                                Navigator.push(context, MaterialPageRoute(builder: (_) => ChatScreen(
                                  id: 0,
                                  name: Provider.of<SplashController>(context, listen: false).configModel?.inHouseShop?.name, userType: 1,
                                  image: "${Provider.of<SplashController>(context, listen: false).configModel?.inHouseShop?.imageFullUrl?.path}",
                                  isShopOnVacation: isVacationActive,
            ...
            Padding(padding: const EdgeInsets.symmetric(vertical: Dimensions.paddingSizeDefault, horizontal:  Dimensions.paddingSizeLarge),
              child: InkWell(onTap: (){
                ...
                child: Container(width: MediaQuery.of(context).size.width, height: 40,
                  ...
                  Theme.of(context).colorScheme.tertiaryContainer.withValues(alpha: 0.20),
